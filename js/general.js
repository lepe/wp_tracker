...
function pastDateUTC(days) {
	var date = new Date(); 
	var past = new Date(date.setDate(date.getDate()-days));
	return new Date(past.getUTCFullYear(),past.getUTCMonth(),past.getUTCDate());
}
//http://stackoverflow.com/questions/11716178/sum-of-multiple-array-elements
function arraysSum(/*arrays list*/){
  var total=[],undefined;
